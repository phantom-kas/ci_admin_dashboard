...
const imageUrl = ref('')
const formData = ref({ description: '', title: '', price: 0, duration: '', instructor: '', file: '' as any });
...
onMounted(async () => {
    ...
    await getTrack().finally(() => {

    })
        .then(res => {
            if (res.data.status != 'success') return
            formData.value.description = res.data.data[0].description
            formData.value.instructor = res.data.data[0].Instructor
            formData.value.price = res.data.data[0].price
            formData.value.title = res.data.data[0].name
            formData.value.duration = res.data.data[0].duration
        })
    hasLoaded.value = true
})
