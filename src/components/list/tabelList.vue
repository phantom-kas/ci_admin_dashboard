...
<template>
    <div class="w-full flex-col flex gap-y-4 @container">
        ...
        <paginationList @fullList="e=>emit('fullList',e)" id="p1" :class="$attrs" :url :params :id-param :component :items-per-page
            ...
            <template #items="{ item, i }">
                <tr :class="{'bg-blue-500 text-black':item._selected ,'odd:bg-neutral-100 odd:dark:bg-neutral-900 @[1px]:bg-[unset]! @[1px]:odd:bg-[unset]':!item._selected}"
                    ...
                    <th v-if="showSelect" scope="col" class="p-4">
                        <div class="flex items-center">
                            <input @change="e=>item._selected? item._selected= false:item._selected = true" id="checkbox-all-search" type="checkbox"
                    ...
                    <td class="px-6 py-3 @[1px]:flex  @[1px]:flex-row justify-between items-center"
                        @click="$emit('clicked', item)" v-for="n, j in listMapper" :key="j">
                        ...
                        <slot v-else :name="n.slotName ?? n.title" v-bind="{ item: item, index: i }">
